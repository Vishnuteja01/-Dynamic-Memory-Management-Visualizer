...
# Description:
# This project implements a tool to simulate and visualize memory management techniques such as Paging, Segmentation, and Virtual Memory.
# It will handle user-defined inputs for memory allocation, page faults, and replacement algorithms (FIFO, LRU).
# We will use Python's standard libraries, matplotlib for visualization, and Tkinter for the user interface.
...
def virtual_memory_simulation(pages: List[int], frame_count: int) -> Tuple[List[List[int]], int]:
    memory = []
    page_faults = 0
    history = []
...
def run_virtual_memory():
    ...
    vm_history, vm_faults = virtual_memory_simulation(pages, frame_count)
    visualize_memory(vm_history, 'Virtual Memory')
    messagebox.showinfo('Virtual Memory Page Faults', f'Total Page Faults (VM): {vm_faults}')
...
btn_virtual_memory = tk.Button(root, text='Run Virtual Memory', command=run_virtual_memory)
btn_virtual_memory.grid(row=6, columnspan=2)
